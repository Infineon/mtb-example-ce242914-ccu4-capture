# PSOC&trade; Control C1 MCU: CCU4 capture

This code example demonstrates the capture features of the capture and compare unit 4 (CCU4) slices available in PSOC&trade; Control C1 MCU.
...
[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce242914-ccu4-capture)

[Provide feedback on this code example.](https://yourvoice.infineon.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
Make the following connections on the board before programming to connect CAPTURE input to PWM output:

**Table 1. Hardware pin connections**

 Board name               | PWM pin    | CAPTURE pin | 
 ------------------------ | ---------- | ----------  | 
 KIT_PSC1M3_EVK           | P0.0       | P4.4        |
...
The following example clones the "[CCU4 capture](https://github.com/Infineon/mtb-example-ce242914-ccu4-capture)" application with the desired name "CCU4 capture" configured for the *KIT_PSC1M3_EVK* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSC1M3_EVK --app-id mtb-example-ce242914-ccu4-capture --user-app-name CCU4 capture --target-dir "C:/mtb_projects"
   ```
...
The application uses two CCU4 slices configured using the CCU4 personality:
   ...
   2. CAPTURE_0 CCU4 slice configured in capture mode. The capture input signal is connected to the output of the PWM_0. It captures the timer value on the rising and falling edges of the generated PWM signal above
...
|CCU4 0 (CC4 0) (PDL)	|PWM_0	                |CCU4 slice configured as PWM generator (2 Hz, 75% duty cycle)                          |
|CCU4 1 (CC4 0) (PDL)	|CAPTURE_0	        |CCU4 slice configured in capture mode (captures timer value on rising/falling edges)   |
...
Document title: *CE242914* – *PSOC&trade; Control C1 MCU: CCU4 capture*
